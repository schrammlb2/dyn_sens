...
# task_list = ['walker', 'halfcheetah', 'ant', 'swimmer', 'hopper', 'humanoid']
name_dict = {}
name_dict['inverted_pendulum'] = 'InvertedPendulumPyBulletEnv-v0'
name_dict['hopper'] = 'HopperPyBulletEnv-v0'
name_dict['halfcheetah'] = 'HalfCheetahPyBulletEnv-v0'
name_dict['walker'] = 'Walker2DPyBulletEnv-v0'
name_dict['ant'] = 'AntPyBulletEnv-v0'
# name_dict['swimmer'] = 'SwimmerPyBulletEnv-v0'
name_dict['humanoid'] = 'HumanoidPyBulletEnv-v0'
...
for key in name_dict.keys():
  ...
  env_dict[key]= (name, 'roboschool_envs/' + key + '/', state_dim, action_dim)

def test_xml_loading(env_name, path):
  filenames = [os.getcwd() + '/'+ path + xml for xml in os.listdir(path) if xml.endswith('.xml') ]
  for filename in filenames:
    with torch.no_grad():
      env = Env(env_name, xml_file=filename)
...
for desc in env_dict.values():
  ...
  env_name= desc[0]
  test_xml_loading(env_name, path)
