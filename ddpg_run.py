...
def train_ddpg(penalty = True, epsilon=.03):
  ...
  for step in pbar:
    ...
    if step > UPDATE_START and step % UPDATE_INTERVAL == 0:
      ...
      # Compute targets
      if penalty:
        tc = gradient_penalty(target_critic, batch['next_state'], target_actor(batch['next_state']))
      else: 
        tc = target_critic(batch['next_state'], target_actor(batch['next_state']))
      y = batch['reward'] + DISCOUNT * (1 - batch['done']) * tc
    ...
    if step > UPDATE_START and step % TEST_INTERVAL == 0:
      ...
      plot(step, total_reward, 'ddpg' + ('_penalty' if penalty else ''))
      actor.train()

# train_ddpg(penalty=False)
train_ddpg(penalty=True)