...
def train_td3(penalty = False, epsilon=.03, actor_LEARNING_RATE=LEARNING_RATE, critic_LEARNING_RATE=LEARNING_RATE):
  ...
  for step in pbar:
    ...
    if step > UPDATE_START and step % TEST_INTERVAL == 0:
      ...
      # plot(step, total_reward, 'td3'+'_'+env_name+('_penalty' if penalty else ''))
      actor.train()
  return sum(rewards)


def objective(trial):
  epsilon = trial.suggest_loguniform('epsilon', 3e-4, .15)
  actor_LEARNING_RATE = LEARNING_RATE#trial.suggest_loguniform('epsilon', LEARNING_RATE*.3, LEARNING_RATE*3)
