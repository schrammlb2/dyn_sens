...
class ActorCritic(nn.Module):
  def __init__(self, hidden_size, state_dim=3, action_dim=1):
    super().__init__()
    self.actor = Actor(hidden_size, stochastic=True, state_dim=3, action_dim=1)
    self.critic = Critic(hidden_size, state_dim=3, action_dim=1)
...
class DQN(nn.Module):
  def __init__(self, hidden_size, num_actions=5, state_dim=3, action_dim=1):
    ...
    super().__init__()
    layers = [nn.Linear(state_dim, hidden_size), nn.Tanh(), nn.Linear(hidden_size, hidden_size), nn.Tanh(), nn.Linear(hidden_size, num_actions)]
